...
class BasicDataset(Dataset):
    def __init__(self, imgs_dir, masks_dir, scale=1, mask_suffix='',data_num = 1):
    ...
    def preprocess(cls, pil_img, scale):
        ...
        #print("max1",np.max(pil_img))

        pil_img = pil_img.resize((newW, newH))
        #print("max",np.max(pil_img))
        img_nd = np.array(pil_img)
        img_nd = np.where(img_nd>1,1.0,img_nd)
    ...
    def __getitem__(self, i):
        ...
        mask = np.load(mask_file[0])["label"]
        mask = np.where((mask == self.data_num),1.0,0.0)
        #print("mask = ",np.max(mask))
        #mask = np.where(mask.astype("float32")==5,1.0,0)
        #print(mask.sum())
        mask = Image.fromarray(mask)
        img = Image.fromarray(np.load(img_file[0])["image"])
